...
// Place holders to allow connecting to other objects
    ...
    // reference gun class to call counter and score
	...
	void Start ()
	{
        //gameOver = player.GetComponent<Gun> ();
	...
	void Update ()
	{
		...
		if (isRunning)
		{
			...
			if(scoreTreasure >= 6 || player.GetComponent<Player>().currentHealth <= 0 || enemyScore >= 14)
			{
				FinishedGame();
			}
	...
	void OnGUI() {
		...
		if(!isRunning)
		{
			...
			{
				message = "Welcome to Sebastian's FPS SHOOTER! Press Enter or Click to play!";
			}
		...
		if(isFinished)
		{
			GUI.Box(new Rect(Screen.width / 2 - 65, 185, 130, 40), "TIME!");
